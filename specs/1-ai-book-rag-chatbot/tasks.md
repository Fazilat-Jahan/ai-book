...
- [ ] T006 [P] Verify Qdrant collection `ai_book_rag_chunks` is created with `Cosine` similarity. Inspect `backend/app/qdrant_client.py` and Qdrant Cloud dashboard.
- [ ] T007 [P] Test basic retrieval from Qdrant to ensure relevant chunks are returned for simple queries. Implement a temporary script for this (e.g., `backend/scripts/test_retrieval.py`).
